...
# endif
  ...
  namespace error
  {
    ...
    // functions with default implementations.  We have to avoid
    // constructing an Fn in this context, (which typically is just a
    // type of a member function) since it may not be possible here in
    // the presence of virtual inheritance.  E.g. gcc complains:
    // error: pointer to member conversion via virtual base 'base'
    //
    ...
    struct virtual_function_default
    {
        ...
        {
	  BOOST_MPL_ASSERT(( mpl::not_<is_same<Default, Fn> > ));

# if !BOOST_WORKAROUND(__MWERKS__, <= 0x2407)
          BOOST_MPL_ASSERT(( is_polymorphic<T> ));
# endif 
          BOOST_MPL_ASSERT(( is_member_function_pointer<Fn> ));

          typedef typename 
            remove_reference<
	      typename mpl::at_c<
          	  typename function_types::components<Fn>::type, 
                  1
                >::type 
              >::type that_type;
	    
          BOOST_MPL_ASSERT(( is_base_of<that_type, T> ));
        }
    };
  }
}
...
class class_ : public objects::class_base
{
 ...
 private: // helper functions
    ...
    {
        return python::make_function(f, 
				     default_call_policies(), 
				     typename detail::get_signature<F, T>::type());
    }
      ...
      log_trace("%s", __PRETTY_FUNCTION__);
        objects::add_to_namespace(
          ...
          , make_function(
              ...
              , helper.policies()
              , helper.keywords()
	      , typename detail::get_signature<Fn, T>::type()
            )
          , helper.doc()
        );
        ...
        , mpl::bool_<true>)
    {
      log_trace("%s", __PRETTY_FUNCTION__);
        ...
        detail::error::virtual_function_default<W,Fn>
	  ::must_be_derived_class_member
    ...
    {
      log_trace("%s", "maybe overloads");
        //  convert sig to a type_list (see detail::get_signature in signature.hpp)
        //  before calling detail::define_with_defaults.
        detail::define_with_defaults(name, 
				     overloads, 
				     *this, 
				     typename detail::get_signature<SigT>::type());
    }
