...
# else 
    python::detail::construct_referent<Ref>(m_result.address(), 
					    (python::detail::borrowed_reference)x);
# endif 
}
...
inline bool object_manager_ref_arg_from_python<Ref>::convertible() const
{
    return detail::object_manager_ref_check(
       python::detail::void_ptr_to_reference(this->m_result.address(), 
					     (Ref(*)())0));
}
...
{
    return python::detail::void_ptr_to_reference(this->m_result.address(), 
						 (Ref(*)())0);
}
