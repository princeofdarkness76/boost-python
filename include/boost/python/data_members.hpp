...
namespace detail
{
  ...
  struct member
  {
    typedef typename value_arg<Data>::type value_arg_t;

    template <typename Sig>
    struct result;

    template <typename Self>
    struct result<Self(Class&, value_arg_t)>
    {
      typedef void type;
    };

    template <typename Self>
    struct result<Self(Class&)>
    {
      typedef Data& type;
    };
    ...
    // setter
    void operator()(Class& c, value_arg_t d) const
    {
      c.*m_which = d;
    }
  private:
  ...
  //
  // A small function object which handles the getting of non-member
  // objects.  This and set_datum are split up to keep result_of from
  // getting confused on a recent gcc.
  //
  template <class Data>
  struct get_datum
  {
    ...
    get_datum(Data *which) : m_which(which) {}
  ...
  struct set_datum
  {
    ...
    set_datum(Data *which) : m_which(which) {}
      
    void operator()(value_arg_t d) const
    {
      *m_which = d;
    }
   private:
  ...
  {
    log_trace("make_getter d=%s policies=%s", name_of(d) % name_of(policies));
      return python::make_function(
          detail::get_datum<D>(d), policies, mpl::vector1<D&>()
      );
  }
  ...
  {
    log_trace("make_getter d=%s policies=%s", name_of(d) % name_of<not_specified>());
  ...
  {
    log_trace("make_getter pm=%s policies=%s", name_of(pm) % name_of(policies));
  ...
  {
    log_trace("make_getter pm=%s Policies=%s", name_of(pm) % name_of<not_specified>());
  ...
  {
    log_trace("make_getter d=%s p=%s", name_of(d) % name_of(p));
  ...
  template <class D, class Policies>
  inline object make_setter(D* p, Policies const& policies, mpl::false_)
  {
    return python::make_function(detail::set_datum<D>(p), 
				 policies, 
  ...
  template <class C, class D, class Policies>
  inline object make_setter(D C::*pm, Policies const& policies, mpl::true_)
  {
  ...
  {
      return detail::make_setter(&x, policies, mpl::false_());
  }
}
...
{
    return detail::make_setter(x, policies, is_member_pointer<D>());
}
...
{
    return detail::make_setter(x, default_call_policies(), is_member_pointer<D>());
}
