...
template <class Sig>
struct signature_base_select<boost::function<Sig> >
{
  enum { arity = boost::function<Sig>::arity };
  typedef typename signature_arity<arity>::template impl<Sig> type;
};

template <typename Arg1, typename Arg2, typename Result>
struct signature_base_select<std::binary_function<Arg1, Arg2, Result> >
{
  enum { arity = 2 };
  //  typedef typename signature_arity<arity>::template impl<Result(Arg1, Arg2)> type;
  typedef void type;
};
