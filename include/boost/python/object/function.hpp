...
struct BOOST_PYTHON_DECL function : PyObject
{
 ...
 private: // helper functions
    ...
    void add_overload(handle<function> const&, bool ambig_check);
    bool has_same_signature_as(function* rhs) const;
    void ambiguous_overload_error(function* rhs);
    void ambiguous_call_error() const;
    void multiple_values_for_keyword_argument_error(std::string keyword) const;
    
 private: // data members
    // we need this one for the "multiple values for keyword argument" check
    unordered_map<std::string, unsigned> keyword_arg_positions;

    typedef std::vector<std::pair<const function*, handle<> > > best_overloads_t;
    mutable best_overloads_t best_overloads;

    typedef optional<unsigned> score_t;
    mutable score_t best_score, this_score;
