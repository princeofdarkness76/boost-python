...
namespace detail
{
  ...
  //  TDS, 20091012
  //
  //  This thing steps in when the mangling_map below is constructed,
  //  and replaces the ugly long basic_string<wchar_t, etc with the
  //  pretty and readable "std::wstring".  If you put more in here, be
  //  sure they are in sorter order on their mangled names (the
  //  'first' of the pair).
  //
  struct demangler_initializer
  {
    demangler_initializer(std::vector<std::pair<char const*, char const*> >& v)
    {
      v.push_back(std::make_pair(typeid(std::wstring).name(), wstring_name));
    }
    const static char wstring_name[];
  };
  const char demangler_initializer::wstring_name[] = "std::wstring";

  BOOST_PYTHON_DECL char const* gcc_demangle(char const* mangled)
  {
      typedef std::vector<
          std::pair<char const*, // mangled
                    char const*> // demangled
      > mangling_map;
      
      static mangling_map demangler;
      static demangler_initializer init(demangler);
