...
...     result = [method(x) for x in values[0]]
...     if result != values[0]:
...         return "Got %s but expected %s" % (result, values[0])
>>> def test_overflow_impl(method, values):
...     for v in values[1]:
...         method(v)
...
>>> test_overflow = shouldthrow(OverflowError, test_overflow_impl)
>>>
...
...     shouldthrow((OverflowError, TypeError), rewrap_value_unsigned_long_long)(v) # throws
...
>>> rewrap_const_reference_bool('yes')  # throws
Traceback (most recent call last):
Boost.Python.ArgumentError: Python argument types in
    builtin_converters_ext.rewrap_const_reference_bool(str)
...
'x'
  ...
  Note that theres currently silent truncation of strings passed to
  char arguments.
...
# print(__name__)
#
# in python3 everything is a long, and that u'syntax'
# doesn't work
#
import sys
if sys.version_info[0] >= 3:  
    sys.modules[__name__].__doc__ = sys.modules[__name__].__doc__.replace('42L', '42')
    sys.modules[__name__].__doc__ = sys.modules[__name__].__doc__.replace('-1L', '-1')
    sys.modules[__name__].__doc__ = sys.modules[__name__].__doc__.replace('0001L', '0001')
    sys.modules[__name__].__doc__ = sys.modules[__name__].__doc__.replace("u'", "'")
